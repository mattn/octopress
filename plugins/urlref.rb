...
module Jekyll
  ...
  class UrlRef < Liquid::Tag
    ...
    def render(context)
      if text = get_cached_text(@url) || get_text_from_url(@url)
        lines = text.split(/\n/, 3)
...
<cite>#{@url}</cite>
    ...
    def cache(url, text)
      cache_file = get_cache_file_for url
    ...
    def get_cache_file_for(url)
      md5 = Digest::MD5.hexdigest url
      File.join @cache_folder, "#{md5}.cache"
    end
    ...
    def get_text_from_url(url)
      ...
      cache url, text unless @cache_disabled
      text
    end
